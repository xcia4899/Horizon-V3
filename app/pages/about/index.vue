<!-- File: pages/about.vue -->
<template>
  <div class="about">
    <!-- Hero -->
    <section class="about-hero">
      <div class="container about-hero-inner">
        <p class="about-hero-kicker">About Horizon V2</p>
        <h1 class="about-hero-title">關於我們</h1>
        <p class="about-hero-subtitle">
          我們專注於電競周邊精選與更好的購物體驗：清楚的分類、好用的篩選、快速的導覽。
        </p>

        <div class="about-hero-actions">
          <NuxtLink class="btn" to="/products">前往商品</NuxtLink>
          <NuxtLink class="btn">聯絡我們</NuxtLink>
        </div>

        <div class="about-hero-highlights" aria-label="quick highlights">
          <div class="about-hero-chip">
            <span class="chip-dot"></span>
            <span>嚴選品牌</span>
          </div>
          <div class="about-hero-chip">
            <span class="chip-dot"></span>
            <span>快速出貨</span>
          </div>
          <div class="about-hero-chip">
            <span class="chip-dot"></span>
            <span>售後支援</span>
          </div>
        </div>
      </div>
    </section>

    <!-- Values -->
    <section class="about-values">
      <div class="container about-values-inner">
        <header class="about-values-head">
          <h2 class="about-values-title">我們的核心價值</h2>
          <p class="about-values-desc">
            更好的資訊呈現，讓使用者在最短時間找需要的商品。
          </p>
        </header>

        <div class="about-values-grid">
          <article
            v-for="item in aboutValues"
            :key="item.title"
            class="about-values-card"
          >
            <div class="about-values-card-icon" aria-hidden="true">
              {{ item.icon }}
            </div>
            <h3 class="about-values-card-title">{{ item.title }}</h3>
            <p class="about-values-card-desc">{{ item.desc }}</p>
          </article>
        </div>
      </div>
    </section>

    <!-- Timeline -->
    <section class="about-timeline">
      <div class="container about-timeline-inner">
        <header class="about-timeline-head">
          <h2 class="about-timeline-title">開發歷史</h2>
          <p class="about-timeline-desc">每一次更新都能有一定程度的升級。</p>
        </header>

        <ol class="about-timeline-list">
          <li
            v-for="m in milestones"
            :key="m.title"
            class="about-timeline-item"
          >
            <div class="chip-dot" aria-hidden="true"></div>
            <div class="about-timeline-content">
              <div class="about-timeline-meta">
                <span class="about-timeline-badge">{{ m.year }}</span>
                <h5 class="about-timeline-meta-title">{{ m.title }}</h5>
              </div>
              <p class="about-timeline-meta-desc">{{ m.desc }}</p>
            </div>
          </li>
        </ol>
      </div>
    </section>

    <!-- Support: FAQ + CTA -->
    <section class="about-support">
      <div class="container about-support-inner">
        <header class="about-support-head">
          <h2 class="about-support-title">網站開發 FAQ</h2>
          <p class="about-support-desc">
            這裡整理了關於 Horizon 網站製作與開發過程的一些問題。
          </p>
        </header>

        <div class="about-faq">
          <details
            v-for="(f, idx) in faqs"
            :key="f.q"
            class="about-faq-item"
            :open="idx === 0"
          >
            <summary class="about-faq-q">
              <span>{{ f.q }}</span>
              <span class="about-faq-icon" aria-hidden="true">▾</span>
            </summary>
            <div class="about-faq-a">
              <p>{{ f.a }}</p>
            </div>
          </details>
        </div>

        <div class="about-cta">
          <div class="about-cta-content">
            <h3 class="about-cta-title">需要協助挑選？</h3>
            <p class="about-cta-desc">
              把預算、用途、喜好告訴我們，我們會提供搭配建議。
            </p>
          </div>
          <div class="about-cta-actions">
            <NuxtLink class="btn" to="/">聯絡我們</NuxtLink>
            <NuxtLink class="btn" to="/products">直接逛逛</NuxtLink>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script setup lang="ts">
useHead({
  title: "關於我們 | Horizon V2",
  meta: [
    {
      name: "description",
      content:
        "Horizon V2 關於我們：電競周邊精選、清楚分類與篩選、快速導覽，讓你更快找到適合的裝備。",
    },
  ],
});

const aboutValues = [
  {
    icon: "⚡",
    title: "效率",
    desc: "更快找到商品：分類、搜尋與篩選設計。",
  },
  {
    icon: "🧩",
    title: "清楚",
    desc: "規格與特色整理成易讀資訊。",
  },
  {
    icon: "🛡️",
    title: "安全",
    desc: "出貨與售後流程透明，遇到問題可快速協助處理。",
  },
  {
    icon: "🎛️",
    title: "可用性",
    desc: "重視 RWD 與操作手感，讓桌機與手機都有良好的使用體驗。",
  },
] as const;

const milestones = [
  {
    year: "2025-06",
    title: "Horizon專案起步",
    desc: "從 Figma 設計稿開始規劃網站，以設計經驗為基礎從零開始設計網站。",
  },
  {
    year: "2025-07",
    title: "Horizon 初版",
    desc: "完成首頁、商品列表、篩選系統、商品詳細頁與購物車頁面，包含完整的 RWD 響應式版型。",
  },
  {
    year: "2026-01",
    title: "Horizon V2 開發",
    desc: "採用 Vue 3 與 Nuxt 重新設計網站架構，整理語意化命名與專案結構，透過模組化開發優化元件拆分與維護性，並建立黑白模式的主題顏色系統。",
  },
  {
    year: "2026-02",
    title: "Horizon V2 上線",
    desc: "整合 Horizon 初版核心功能並完成正式上線，針對效能進行優化（包含資料處理與頁面渲染），提升整體載入速度與操作流暢度；同時導入 UI 元件庫（Element Plus）加速開發流程，補齊初版未實作的功能並調整設計細節，持續優化 RWD 與整體使用者體驗。",
  },
  {
    year: "Next",
    title: "會員與後台",
    desc: "預計導入 Supabase 建立會員系統（登入／註冊／驗證）與資料儲存，並以 Vercel 部署應用；同時設計 RESTful API 串接前後端，實作訂單管理與後台內容管理功能，完善電商平台的資料流與整體架構。",
  },
] as const;

const faqs = [
  {
    q: "Horizon 是使用什麼技術製作的？",
    a: "網站從 Vue 2 CDN 開始建構，逐步轉向以 Vue 3 與 Nuxt 為核心框架進行重構，並透過元件化與模組化方式建立清晰且易於維護的整體網站架構。",
  },
  {
    q: "這個網站是如何規劃與設計的？",
    a: "專案從 Figma 介面設計開始，先規劃整體版面與使用流程，再實作成可互動的靜態網站。",
  },
  {
    q: "網站有哪些前端功能實作？",
    a: "包含商品列表、篩選系統、搜尋、分頁、商品詳細頁與響應式版型等功能，並持續增加功能中。",
  },
  {
    q: "這個專案的開發目的是什麼？",
    a: "主要作為前端開發的核心實作專案，除用於練習前端框架、網站架構設計與電商介面開發外，亦作為個人代表作品，展現完整開發流程與實務應用能力。",
  },
] as const;
</script>

<style scoped lang="scss">
.about {
  --shadow: 0 14px 40px rgba(0, 0, 0, 0.35);
  background: var(--bg-surface);
}
/* ========= layout ========= */
.container {
  width: min(1024px, calc(100% - 32px));
  margin: 0 auto;
}
.chip-dot {
  width: 8px;
  height: 8px;
  border-radius: 99px;
  background: transparent;
  box-shadow: 0 0 0 4px rgba($color-purple-500, 0.6);
}
/* ========= Hero ========= */
.about-hero {
  position: relative;
  padding: clamp(96px, 12vw, 128px) 0 36px;
  overflow: hidden;
  border-bottom: 1px solid var(--border-default);
  background:
    linear-gradient(180deg, var(--bg-surface-soft), transparent 20%),
    radial-gradient(
      700px 260px at 20% 0%,
      rgba(#cb8cf5, 0.18),
      transparent 60%
    ),
    rgba(#690fa6, 0.03);

  .about-hero-inner {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    gap: 8px;
    // padding-inline: 16px;
  }
  .about-hero-kicker {
    padding: 0px 12px;
    // border-radius: 8px;
    // background: var(--brand);
    // color: $color-white;
    font-size: 12px;
  }
  .about-hero-title {
    font-size: clamp(34px, 5vw, 56px);
    margin: 0;
  }

  .about-hero-subtitle {
    color: var(--text-secondary);
    line-height: 1.8;
  }
  .about-hero-actions {
    display: flex;
    gap: 16px;
    margin-top: 16px;
    flex-wrap: wrap;
  }

  .about-hero-highlights {
    display: flex;
    gap: 8px;
    flex-wrap: wrap;
  }

  .about-hero-chip {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    border-radius: 16px;
    background: var(--bg-surface);
    border: 1px solid var(--border-default);
    color: var(--text-secondary);
    font-size: 13px;
  }
}

/* ========= Values ========= */
.about-values {
  padding: clamp(36px, 5vw, 72px) 0;
  background: var(--bg-surface-strong);

  .about-values-inner {
    display: grid;
    gap: 16px;
  }
  .about-values-head {
    display: grid;
    gap: 4px;
    .about-values-desc {
      color: var(--text-secondary);
    }
  }
  .about-values-grid {
    display: grid;
    gap: 16px;
    grid-template-columns: repeat(4, 1fr);
    @media (max-width: 980px) {
      grid-template-columns: repeat(2, 1fr);
    }
    @media (max-width: 550px) {
      grid-template-columns: 1fr;
    }

    .about-values-card {
      background: var(--bg-surface);
      border: 2px solid var(--border-default);
      border-radius: 8px;
      padding: 16px;
      box-shadow: 0 10px 30px rgba(41, 41, 41, 0.18);

      display: grid;
      gap: 8px;

      transition: border-color 0.6s ease;
      .about-values-card-icon {
        width: 40px;
        height: 40px;
        border-radius: 8px;
        display: grid;
        place-items: center;
        background: var(--bg-surface-strong);
        border: 1px solid var(--border-default);
        font-size: clamp(14px, 2vw, 18px);
      }
      .about-values-card-title {
        font-weight: 700;
      }
      .about-values-card-desc {
        color: var(--text-secondary);
      }
      @media (hover: hover) and (pointer: fine) {
        &:hover {
          border-color: var(--border-soft);
        }
      }
    }
  }
}

/* ========= Timeline ========= */
.about-timeline {
  padding: clamp(36px, 5vw, 72px) 0;
  border-block: 1px solid var(--border-default);
  .about-timeline-inner {
    display: grid;
    gap: 16px;
  }
  .about-timeline-head {
    display: grid;
    gap: 4px;
    .about-timeline-desc {
      color: var(--text-secondary);
    }
  }
  .about-timeline-list {
    padding: 0;
    display: grid;
    gap: 12px;
    .about-timeline-item {
      display: flex;
      align-items: flex-start;
      gap: 16px;
      .about-timeline-content {
        flex: 1;
        display: grid;
        gap: 8px;
        padding: 16px;
        border-radius: 8px;
        background: var(--bg-surface-soft);
        border: 2px solid var(--border-default);
        transition: border-color 0.6s ease;
        .about-timeline-meta {
          display: flex;
          align-items: center;
          gap: 8px;
          flex-wrap: wrap;
          .about-timeline-badge {
            padding: 4px 8px;
            border-radius: 12px;
            border: 1px solid var(--border-default);
            color: var(--text-secondary);
            background: var(--bg-surface);
            font-size: 12px;
          }
          .about-timeline-meta-title {
            font-weight: 700;
          }
        }
        .about-timeline-meta-desc {
          color: var(--text-secondary);
        }
        @media (hover: hover) and (pointer: fine) {
          &:hover {
            border-color: var(--border-soft);
          }
        }
      }
    }
  }
}

/* ========= Support (FAQ + CTA) ========= */
.about-support {
  padding: clamp(36px, 5vw, 72px) 0;
  background: var(--bg-surface-strong);
  .about-support-inner {
    display: grid;
    gap: 16px;
  }
  .about-support-head {
    display: grid;
    gap: 4px;
    .about-support-desc {
      color: var(--text-secondary);
    }
  }

  /* FAQ */
  .about-faq {
    display: grid;
    gap: 8px;
    .about-faq-item {
      border-radius: 8px;
      border: 2px solid var(--border-default);
      background: var(--bg-surface);
      overflow: hidden;

      .about-faq-q {
        cursor: pointer;
        padding: 16px;
        display: flex;
        justify-content: space-between;
        align-items: center;
        gap: 12px;
        user-select: none;
      }
      .about-faq-icon {
        font-size: 20px;
        transition: transform 0.4s ease;
        color: var(--text-secondary);
      }
      &[open] {
        border-color: var(--brand-hover);
        .about-faq-icon {
          transform: rotate(180deg) scale(2);
          color: var(--brand-hover);
        }
      }
      .about-faq-a {
        padding: 0 16px 12px;
        color: var(--text-secondary);
      }
    }
  }

  /* CTA */
  .about-cta {
    margin-top: 16px;
    padding: 16px 60px;
    border-radius: 20px;
    border: 1px solid rgba(#cb8cf5, 0.8);
    background:
      radial-gradient(
        700px 260px at 20% 0%,
        rgba(#cb8cf5, 0.18),
        transparent 60%
      ),
      rgba(#690fa6, 0.03);
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 14px;
    flex-wrap: wrap;

    .about-cta-title {
      font-size: 18px;
      font-weight: 800;
    }

    .about-cta-desc {
      margin-top: 6px;
      color: var(--text-secondary);
      line-height: 1.8;
    }

    .about-cta-actions {
      display: flex;
      gap: 10px;
      flex-wrap: wrap;
    }
  }
}
</style>
